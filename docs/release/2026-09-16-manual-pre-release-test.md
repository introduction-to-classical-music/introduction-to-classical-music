...
11. 合并或导入完成后，维护工具会自动重载当前服务；再点击“构建不全书”或“打开不全书”，必须看到刚刚合并的活动库。若结果区出现 `Failed to fetch`，说明维护工具窗口未重载，应关闭并重新打开维护工具后记录该问题。
...
1. 保持官方 library 为当前库。
2. 点击“导出网站”，直接选择目录 `manual-test\site-export\current`。该按钮只选择目录，不询问 library 载体格式；维护工具会使用相对资源路径 `./`，因此可直接双击 `index.html` 检查样式。
